...
public class PJpanel2 extends JPanel implements MouseMotionListener,MouseListener{
...
//	List<Gift>gifts=new ArrayList<Gift>();
	...
	public PJpanel2(){
		try {
			pImg=ImageIO.read(new File("Image/yxj3.png"));//控制的飞机
		...
		Thread t= new Thread(){
			public void run(){
				while(true){
					...
					if(y==-2000&&ccount==2)
					{
						Boss bs=new Boss(100, 10, (int)(Math.random()*2), 100, bossImg);
						boss.add(bs);
						fb=true;
					...
					if(count%30==0){
						for (int i = 0; i < boss.size(); i++) {
							Boss b=boss.get(i);
							BossBullet bob1=new BossBullet(b.x+bossImg.getWidth(null)/2, b.y+bossImg.getHeight(null)/2,2,boImage);
							bossbullet.add(bob1);
							BossBullet bob2=new BossBullet(b.x+bossImg.getWidth(null)/2-50, b.y+bossImg.getWidth(null)/2-5, 2,boImage);
							bossbullet.add(bob2);
							BossBullet bob3=new BossBullet(b.x+bossImg.getWidth(null)/2+40, b.y+bossImg.getWidth(null)/2-5, 2, boImage);
							bossbullet.add(bob3);
						}
					}
					//boss子弹移动
